...
class TestPreprocess(unittest.TestCase):
    def setUp(self):
        ...
        self.ans_train_features_preprocess = pd.read_csv(os.path.join("task2","train_feats_preprocess.csv")).set_index("index")
        self.ans_test_features_preprocess = pd.read_csv(os.path.join("task2","test_feats_preprocess.csv")).set_index("index")
